...
# 4. LLM Node with System Guidance (Listing 11.11 - Task 4)
def llm_node(state: AgentState):
    """LLM node that decides whether to call search/weather tools or return a final answer."""
    current_messages = list(state["messages"])
    system_message = SystemMessage(
        content="""You are a helpful travel assistant that can search travel information and get the weather forecast.
Only use the tools to find the information you need (including town names). Do not rely solely on pre-trained knowledge."""
    )
...
# 5. Assembling Multi-Tool Agent Graph
...
# 6. Chatbot REPL Loop
def chat_loop():
    print("=== UK Travel Assistant (Task 4: Multi-Tool Agent with System Guidance) ===")
